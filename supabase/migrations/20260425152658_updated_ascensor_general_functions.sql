drop function if exists "public"."get_global_classification"();

drop function if exists "public"."get_month_classification"(p_mes_id integer, p_categoria_id integer);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.get_global_classification(p_temporada_id integer)
 RETURNS TABLE(player_id integer, nickname text, points integer, diff integer, games_for integer, matches_played integer)
 LANGUAGE sql
 STABLE
AS $function$
WITH sets_base AS (
